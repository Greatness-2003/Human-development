...
For the geo-referenced data obtained for this study, a logistic regression model as a special case of the Generalized Linear Mixed Models (GLMMs) that include all variables of interest generated spatially auto-correlated residuals (Moran’s I=0.233, P<0.0001). Here, the GLMM framework to fit the under-5 malaria prevalence data using the logit link function was employed as follows: 

   + + + + + + + + +  ![img10](https://user-images.githubusercontent.com/68754608/142742371-ff89a67a-d3fa-4273-81c9-987340824aac.jpg)
   
The predicted values from the model were mapped to obtain risk maps for the under-5 malaria infections at the national level. This was achieved by generating the fitted values of the response variable predicted by the S-GLMM fitted to the data, on which they applied ordinary Kriging to infer values at unobserved locations in the proximity of the data points. This approach enabled modelling and mappping the risk of under-5 malaria to identify critical hotspots of malaria clusters. The spatial analysis was carried out using ArcGIS and the statistical analysis was implemented using SAS statistical software. The significance level in the analysis was P=0.05.
